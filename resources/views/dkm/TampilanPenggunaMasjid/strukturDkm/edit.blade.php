...
@section('title', 'Edit Anggota DKM')
...
<div class="bg-white p-4 sm:p-6 rounded-lg shadow-md">
    ...
    {{-- Header --}}
    <div class="mb-6">
        <h2 class="text-2xl font-bold text-gray-800">Edit Anggota Struktur DKM</h2>
        <p class="text-gray-600">Perbarui data anggota di bawah ini.</p>
    </div>

    {{-- Form --}}
    <form id="strukturDkmForm" action="{{ route('dkm.tampilanPenggunaMasjid.strukturDkm.update', $strukturDkm->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-4">
                ...
                <div>
                    ...
                    <input type="text" id="nama" name="nama" value="{{ old('nama', $strukturDkm->nama) }}"
                           class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500"
                           placeholder="Contoh: H. Muhammad Syaifulloh" required>
                    @error('nama')
                ...
                <div>
                    ...
                    <input type="text" id="divisi" name="divisi" value="{{ old('divisi', $strukturDkm->divisi) }}"
                           class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500"
                           placeholder="Contoh: Ketua DKM" required>
                    @error('divisi')
                        <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div class="lg:col-span-1 space-y-2">
                ...
                <div class="w-full">
                    <img id="img-preview" 
                         src="{{ $strukturDkm->gambar ? asset('images/struktur_dkm/' . $strukturDkm->gambar) : asset('images/person-icon.png') }}" 
                ...
                <label for="gambar" class="cursor-pointer w-full inline-block text-center bg-white border border-gray-300 text-gray-700 px-4 py-2 rounded-lg shadow-sm hover:bg-gray-50 transition duration-300">
                    <i class="fas fa-upload mr-2"></i>Ganti Gambar (Opsional)
                </label>
...
<script>
    ...
    // Validasi Sisi Klien (Hanya untuk field yang required)
